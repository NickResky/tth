      ...
      .then((zenkitListData) => {
        const mainPageSections = _.map(
          ...
          (modifiedEntry) => {
            ...
            mainPageSection.speechBubble = modifiedEntry.speechBubble.length
              ? modifiedEntry.speechBubble
              : undefined;
            mainPageSection.speechBubbleImage = _.head(
              modifiedEntry.speechBubbleImage
            );
        ...
        const getSection = function (sectionName) {
          const section = _.head(
            _.remove(mainPageSections, {
              title: sectionName,
            })
          );
        ...
        const ernaehrungsberatungSection = getSection("Ernährungs-Beratung");
